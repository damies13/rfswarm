...
Verify the Result Name Is Returned From Keywords Correctly DEFAULT
	[Tags]	windows-latest	ubuntu-latest	macos-latest	Issue #56
	[Setup] 	Run Keywords 	Set INI Window Size		1200	600 	AND 	Open Agent

	VAR 	${scenario_name} 	Issue-#56_default
	${scenariofile}= 	Normalize Path 	${CURDIR}${/}testdata${/}Issue-#56${/}${scenario_name}.rfs
	Log 	${EMPTY} 	console=True
	Log 	scenariofile: ${scenariofile} 	console=True

	Log 	Test DEFAULT Result Name Option. 	console=${True}
	...
	Log To Console 	${\n}===[ Check Database for result names from keywords. ]===
	${dbfile}= 	Find Result DB 		result_pattern=*_${scenario_name}*
	${db_results}= 	Query Result DB 	${dbfile}
	...    SELECT DISTINCT Name FROM ResultSummary ORDER BY Name
	Log 	Found result names from keywords in ResultSummary table after run: ${db_results}	console=${True}
	Length Should Be 	${db_results} 	4 	msg=Unique Result Names number from keywords is incorrect. Check the Logs.
	Should Be Equal 	${db_results}[0][0] 	Default Keyword Documentation 		msg=Can't find expected result name from keywords for DEFAULT Option
	Should Be Equal 	${db_results}[1][0] 	Doc Keyword Message 				msg=Can't find expected result name from keywords for DEFAULT Option
	Should Be Equal 	${db_results}[2][0] 	Doc only keyword From Info Library 	msg=Can't find expected result name from keywords for DEFAULT Option
	Should Be Equal 	${db_results}[3][0] 	Message for Info Keyword 			msg=Can't find expected result name from keywords for DEFAULT Option

	[Teardown]	Run Keywords	Close Manager GUI ${platform} 	AND 	Stop Agent

Verify the Result Name Is Returned From Keywords Correctly DOCUMENTATION
	[Tags]	windows-latest	ubuntu-latest	macos-latest	Issue #56
	[Setup] 	Run Keywords 	Set INI Window Size		1200	600 	AND 	Open Agent

	VAR 	${scenario_name} 	Issue-#56_documentation
	${scenariofile}= 	Normalize Path 	${CURDIR}${/}testdata${/}Issue-#56${/}${scenario_name}.rfs
	Log 	${EMPTY} 	console=True
	Log 	scenariofile: ${scenariofile} 	console=True

	Log 	Test DOCUMENTATION Result Name Option. 	console=${True}
	VAR 	@{mngr_options} 	-s  ${scenariofile}  -d  ${results_dir}
	Open Manager GUI	${mngr_options}
	Check If The Agent Is Ready
	Click Tab	Plan
	...
	${dbfile}= 	Find Result DB 		result_pattern=*_${scenario_name}*
	${db_results}= 	Query Result DB 	${dbfile}
	...    SELECT DISTINCT Name FROM ResultSummary
	Log 	Found result names from keywords in ResultSummary table after run: ${db_results}	console=${True}
	Length Should Be 	${db_results} 	3 	msg=Unique Result Names number from keywords is incorrect. Check the Logs.
	Should Be Equal 	${db_results}[0][0] 	Default Keyword Documentation 		msg=Can't find expected result name from keywords for DOCUMENTATION Option
	Should Be Equal 	${db_results}[1][0] 	Doc keyword From Info Library 		msg=Can't find expected result name from keywords for DOCUMENTATION Option
	Should Be Equal 	${db_results}[2][0] 	Doc only keyword From Info Library 	msg=Can't find expected result name from keywords for DOCUMENTATION Option

	[Teardown]	Run Keywords	Close Manager GUI ${platform} 	AND 	Stop Agent

Verify the Result Name Is Returned From Keywords Correctly INFORMATION
	[Tags]	windows-latest	ubuntu-latest	macos-latest	Issue #56
	[Setup] 	Run Keywords 	Set INI Window Size		1200	600 	AND 	Open Agent

	VAR 	${scenario_name} 	Issue-#56_information
	${scenariofile}= 	Normalize Path 	${CURDIR}${/}testdata${/}Issue-#56${/}${scenario_name}.rfs
	Log 	${EMPTY} 	console=True
	Log 	scenariofile: ${scenariofile} 	console=True

	Log 	Test INFORMATION Result Name Option. 	console=${True}
	VAR 	@{mngr_options} 	-s  ${scenariofile}  -d  ${results_dir}
	Open Manager GUI	${mngr_options}
	Check If The Agent Is Ready
	Click Tab	Plan
	...
	...    SELECT DISTINCT Name FROM ResultSummary ORDER BY Name
	Log 	Found result names from keywords in ResultSummary table after run: ${db_results}	console=${True}
	Length Should Be 	${db_results} 	2 	msg=Unique Result Names number from keywords is incorrect. Check the Logs.
	Should Be Equal 	${db_results}[0][0] 	Doc Keyword Message 		msg=Can't find expected result name from keywords for INFORMATION Option
	Should Be Equal 	${db_results}[1][0] 	Message for Info Keyword 	msg=Can't find expected result name from keywords for INFORMATION Option

	[Teardown]	Run Keywords	Close Manager GUI ${platform} 	AND 	Stop Agent

Verify the Result Name Is Returned From Keywords Correctly KEYWORD
	[Tags]	windows-latest	ubuntu-latest	macos-latest	Issue #56
	[Setup] 	Run Keywords 	Set INI Window Size		1200	600 	AND 	Open Agent

	VAR 	${scenario_name} 	Issue-#56_keyword
	${scenariofile}= 	Normalize Path 	${CURDIR}${/}testdata${/}Issue-#56${/}${scenario_name}.rfs
	Log 	${EMPTY} 	console=True
	Log 	scenariofile: ${scenariofile} 	console=True

	Log 	Test KEYWORD Result Name Option. 	console=${True}
	VAR 	@{mngr_options} 	-s  ${scenariofile}  -d  ${results_dir}
	Open Manager GUI	${mngr_options}
	Check If The Agent Is Ready
	Click Tab	Plan
	...
	...    SELECT DISTINCT Name FROM ResultSummary ORDER BY Name
	Log 	Found result names from keywords in ResultSummary table after run: ${db_results}	console=${True}
	Length Should Be 	${db_results} 	7 		msg=Unique Result Names number from keywords is incorrect. Check the Logs.
	Should Be Equal 	${db_results}[0][0] 	Argument Keyword 		msg=Can't find expected result name from keywords for KEYWORD Option
	Should Be Equal 	${db_results}[1][0] 	Default Keyword Name 	msg=Can't find expected result name from keywords for KEYWORD Option
	Should Be Equal 	${db_results}[2][0] 	Doc Keyword 			msg=Can't find expected result name from keywords for KEYWORD Option
	Should Be Equal 	${db_results}[3][0] 	Doc Only Keyword 		msg=Can't find expected result name from keywords for KEYWORD Option
	Should Be Equal 	${db_results}[4][0] 	Info Keyword 			msg=Can't find expected result name from keywords for KEYWORD Option
	Should Be Equal 	${db_results}[5][0] 	Quiet Keyword Name 		msg=Can't find expected result name from keywords for KEYWORD Option
	Should Be Equal 	${db_results}[6][0] 	Return Only Keyword 	msg=Can't find expected result name from keywords for KEYWORD Option

	[Teardown]	Run Keywords	Close Manager GUI ${platform} 	AND 	Stop Agent

Verify the Result Name Is Returned From Keywords Correctly KEYWORD & ARGUMENTS
	[Tags]	windows-latest	ubuntu-latest	macos-latest	Issue #56
	[Setup] 	Run Keywords 	Set INI Window Size		1200	600 	AND 	Open Agent

	VAR 	${scenario_name} 	Issue-#56_keyword_arguments
	${scenariofile}= 	Normalize Path 	${CURDIR}${/}testdata${/}Issue-#56${/}${scenario_name}.rfs
	Log 	${EMPTY} 	console=True
	Log 	scenariofile: ${scenariofile} 	console=True

	Log 	Test KEYWORD & ARGUMENTS Result Name Option. 	console=${True}
	VAR 	@{mngr_options} 	-s  ${scenariofile}  -d  ${results_dir}
	Open Manager GUI	${mngr_options}
	Check If The Agent Is Ready
	Click Tab	Plan
	...
	Log 	Found result names from keywords in ResultSummary table after run: ${db_results}	console=${True}
	Length Should Be 	${db_results} 	7 	msg=Unique Result Names number from keywords is incorrect. Check the Logs.
	Should Be Equal 	${db_results}[0][0] 	Argument Keyword Arg1
	...    msg=Can't find expected result name from keywords for KEYWORD & ARGUMENTS Option Option
	Should Be Equal 	${db_results}[1][0] 	Default Keyword Name
	...    msg=Can't find expected result name from keywords for KEYWORD & ARGUMENTS Option
	Should Be Equal 	${db_results}[2][0] 	Doc Keyword Doc Keyword Message
	...    msg=Can't find expected result name from keywords for KEYWORD & ARGUMENTS Option
	Should Be Equal 	${db_results}[3][0] 	Doc Only Keyword Doc Keyword Message
	...    msg=Can't find expected result name from keywords for KEYWORD & ARGUMENTS Option
	Should Be Equal 	${db_results}[4][0] 	Info Keyword Message for Info Keyword
	...    msg=Can't find expected result name from keywords for KEYWORD & ARGUMENTS Option
	Should Be Equal 	${db_results}[5][0] 	Quiet Keyword Name
	...    msg=Can't find expected result name from keywords for KEYWORD & ARGUMENTS Option
	Should Be Equal 	${db_results}[6][0] 	Return Only Keyword Hello
	...    msg=Can't find expected result name from keywords for KEYWORD & ARGUMENTS Option
